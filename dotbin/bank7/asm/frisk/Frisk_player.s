...
.include "Frisk_variables.inc"

;;; Symbols from song track data
.import frisk_tt_TrackDataStart
.import frisk_tt_InsCtrlTable
.import frisk_tt_InsADIndexes
.import frisk_tt_InsSustainIndexes
.import frisk_tt_InsReleaseIndexes
.import frisk_tt_InsFreqVolTable
.import frisk_tt_PercIndexes
.import frisk_tt_PercFreqTable
.import frisk_tt_PercCtrlVolTable
.import frisk_tt_PatternPtrHi
.import frisk_tt_PatternPtrLo
.import frisk_tt_SequenceTable

.export _frisk_tt_player
...
; - ROM: Check if frisk_tt_SequenceTable can hold ptrs directly without indexing
;       frisk_tt_PatternPtrLo/Hi. Can be smaller if not many patterns get repeated
;       (saves table and decode routine)
; - Speed: Inline frisk_tt_CalcInsIndex
...
constructPatPtr:
        ldy _tt_cur_pat_index_c0,x       ; get current pattern (index into frisk_tt_SequenceTable)
        lda frisk_tt_SequenceTable,y
   .IF TT_USE_GOTO = 1
        bpl noPatternGoto
...
noPatternGoto:
   .ENDIF
        tay
        lda frisk_tt_PatternPtrLo,y
        sta ptr1
        lda frisk_tt_PatternPtrHi,y
        sta ptr1+1
        ; get new note
   .IF TT_USE_OVERLAY = 0
...
pause:
        ...
        jsr frisk_tt_CalcInsIndex
        lda frisk_tt_InsReleaseIndexes-1,y    ; -1 b/c instruments start at #1
...
newNote:
        ...
        lda frisk_tt_PercIndexes-TT_FIRST_PERC,y
...
startInstrument:
   ...
   .ENDIF
        ...
        jsr frisk_tt_CalcInsIndex
        lda frisk_tt_InsADIndexes-1,y         ; -1 because instruments start at #1
storeADIndex:
...
sequencerNextChannel:
   ...
   .IF TT_GLOBAL_SPEED = 0
        ...
        ldx _tt_cur_pat_index_c0         ; get current pattern (index into frisk_tt_SequenceTable)
        ldy frisk_tt_SequenceTable,x          ; Current pattern index now in y
     .IF TT_USE_FUNKTEMPO = 0
        lda frisk_tt_PatternSpeeds,y
        sta _tt_timer
     .ELSE
        ; Test for odd/even frame
        lda _tt_cur_note_index_c0
        lsr
        lda frisk_tt_PatternSpeeds,y          ; does not affect carry flag
        bcc evenFrame
...
updateLoop:
   ...
   .ENDIF
        ...
        lda frisk_tt_PercCtrlVolTable-1,y     ; -1 because values are stored +1
...
endOfPercussion:
        ...
        sta AUDC0,x     
        ; Set AUDF
        lda frisk_tt_PercFreqTable-1,y        ; -1 because values are stored +1
   ...
   .IF TT_USE_OVERLAY = 1
        ...
        sta _tt_cur_ins_c0,x             ; set new instrument
        jsr frisk_tt_CalcInsIndex
        lda frisk_tt_InsSustainIndexes-1,y    ; -1 because instruments start at #1
        sta _tt_envelope_index_c0,x      
...
frisk_tt_CalcInsIndex:
        ...
        lsr
        tay
frisk_tt_Bit6Set:     ; This opcode has bit #6 set, for use with bit instruction
        rts
...
instrument:
        ...
        lda frisk_tt_InsCtrlTable-1,y ; -1 because instruments start with #1
        sta AUDC0,x
        ; advance ADSR counter and compare to end of Sustain
        lda _tt_envelope_index_c0,x
        cmp frisk_tt_InsReleaseIndexes-1,y    ; -1 because instruments start with #1
        bne noEndOfSustain
        ; End of sustain: Go back to start of sustain
        lda frisk_tt_InsSustainIndexes-1,y    ; -1 because instruments start with #1
noEndOfSustain:
        tay
        ; Set volume from envelope
        lda frisk_tt_InsFreqVolTable,y
	sta _tt_cur_vol_c0,x
