...
.include "ThundArmand_variables.inc"

;;; Symbols from song track data
.import thund_tt_TrackDataStart
.import thund_tt_InsCtrlTable
.import thund_tt_InsADIndexes
.import thund_tt_InsSustainIndexes
.import thund_tt_InsReleaseIndexes
.import thund_tt_InsFreqVolTable
.import thund_tt_PercIndexes
.import thund_tt_PercFreqTable
.import thund_tt_PercCtrlVolTable
.import thund_tt_PatternPtrHi
.import thund_tt_PatternPtrLo
.import thund_tt_SequenceTable

.export _thund_tt_player
...
; TIATracker Player
; =====================================================================
thund_tt_PlayerStart:

   .MAC TT_FETCH_CURRENT_NOTE
        ; construct ptr to pattern
constructPatPtr:
        ldy _tt_cur_pat_index_c0,x       ; get current pattern (index into thund_tt_SequenceTable)
        lda thund_tt_SequenceTable,y
   .IF TT_USE_GOTO = 1
        bpl noPatternGoto
...
noPatternGoto:
   .ENDIF
        tay
        lda thund_tt_PatternPtrLo,y
        sta ptr1
        lda thund_tt_PatternPtrHi,y
        sta ptr1+1
        ; get new note
   .IF TT_USE_OVERLAY = 0
...
pause:
        ...
        jsr thund_tt_CalcInsIndex
        lda thund_tt_InsReleaseIndexes-1,y    ; -1 b/c instruments start at #1
...
newNote:
        ...
        lda thund_tt_PercIndexes-TT_FIRST_PERC,y
...
startInstrument:
   ...
   .ENDIF
        ...
        jsr thund_tt_CalcInsIndex
        lda thund_tt_InsADIndexes-1,y         ; -1 because instruments start at #1
storeADIndex:
...
sequencerNextChannel:
   ...
   .IF TT_GLOBAL_SPEED = 0
        ...
        ldx _tt_cur_pat_index_c0         ; get current pattern (index into thund_tt_SequenceTable)
        ldy thund_tt_SequenceTable,x          ; Current pattern index now in y
     .IF TT_USE_FUNKTEMPO = 0
        lda thund_tt_PatternSpeeds,y
        sta _tt_timer
     .ELSE
        ; Test for odd/even frame
        lda _tt_cur_note_index_c0
        lsr
        lda thund_tt_PatternSpeeds,y          ; does not affect carry flag
        bcc evenFrame
...
updateLoop:
   ...
   .ENDIF
        ...
        lda thund_tt_PercCtrlVolTable-1,y     ; -1 because values are stored +1
...
endOfPercussion:
        ...
        sta AUDC0,x     
        ; Set AUDF
        lda thund_tt_PercFreqTable-1,y        ; -1 because values are stored +1
   ...
   .IF TT_USE_OVERLAY = 1
        ...
        sta _tt_cur_ins_c0,x             ; set new instrument
        jsr thund_tt_CalcInsIndex
        lda thund_tt_InsSustainIndexes-1,y    ; -1 because instruments start at #1
        sta _tt_envelope_index_c0,x      
...
thund_tt_CalcInsIndex:
        ...
        lsr
        tay
thund_tt_Bit6Set:     ; This opcode has bit #6 set, for use with bit instruction
        rts
...
instrument:
        ...
        lda thund_tt_InsCtrlTable-1,y ; -1 because instruments start with #1
        sta AUDC0,x
        ; advance ADSR counter and compare to end of Sustain
        lda _tt_envelope_index_c0,x
        cmp thund_tt_InsReleaseIndexes-1,y    ; -1 because instruments start with #1
        bne noEndOfSustain
        ; End of sustain: Go back to start of sustain
        lda thund_tt_InsSustainIndexes-1,y    ; -1 because instruments start with #1
noEndOfSustain:
        tay
        ; Set volume from envelope
        lda thund_tt_InsFreqVolTable,y
	sta _tt_cur_vol_c0,x
