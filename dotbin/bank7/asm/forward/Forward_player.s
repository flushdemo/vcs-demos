...
.include "Forward_variables.inc"

;;; Symbols from song track data
.import forward_tt_TrackDataStart
.import forward_tt_InsCtrlTable
.import forward_tt_InsADIndexes
.import forward_tt_InsSustainIndexes
.import forward_tt_InsReleaseIndexes
.import forward_tt_InsFreqVolTable
.import forward_tt_PercIndexes
.import forward_tt_PercFreqTable
.import forward_tt_PercCtrlVolTable
.import forward_tt_PatternPtrHi
.import forward_tt_PatternPtrLo
.import forward_tt_SequenceTable

.export _forward_tt_player
...
; - ROM: Check if forward_tt_SequenceTable can hold ptrs directly without indexing
;       forward_tt_PatternPtrLo/Hi. Can be smaller if not many patterns get repeated
;       (saves table and decode routine)
; - Speed: Inline forward_tt_CalcInsIndex
...
constructPatPtr:
        ldy _tt_cur_pat_index_c0,x       ; get current pattern (index into forward_tt_SequenceTable)
        lda forward_tt_SequenceTable,y
   .IF TT_USE_GOTO = 1
        bpl noPatternGoto
...
noPatternGoto:
   .ENDIF
        tay
        lda forward_tt_PatternPtrLo,y
        sta ptr1
        lda forward_tt_PatternPtrHi,y
        sta ptr1+1
        ; get new note
   .IF TT_USE_OVERLAY = 0
...
pause:
        ...
        jsr forward_tt_CalcInsIndex
        lda forward_tt_InsReleaseIndexes-1,y    ; -1 b/c instruments start at #1
...
newNote:
        ...
        lda forward_tt_PercIndexes-TT_FIRST_PERC,y
...
startInstrument:
   ...
   .ENDIF
        ...
        jsr forward_tt_CalcInsIndex
        lda forward_tt_InsADIndexes-1,y         ; -1 because instruments start at #1
storeADIndex:
...
sequencerNextChannel:
   ...
   .IF TT_GLOBAL_SPEED = 0
        ...
        ldx _tt_cur_pat_index_c0         ; get current pattern (index into forward_tt_SequenceTable)
        ldy forward_tt_SequenceTable,x          ; Current pattern index now in y
     .IF TT_USE_FUNKTEMPO = 0
        lda forward_tt_PatternSpeeds,y
        sta _tt_timer
     .ELSE
        ; Test for odd/even frame
        lda _tt_cur_note_index_c0
        lsr
        lda forward_tt_PatternSpeeds,y          ; does not affect carry flag
        bcc evenFrame
...
updateLoop:
   ...
   .ENDIF
        ...
        lda forward_tt_PercCtrlVolTable-1,y     ; -1 because values are stored +1
...
endOfPercussion:
        ...
        sta AUDC0,x     
        ; Set AUDF
        lda forward_tt_PercFreqTable-1,y        ; -1 because values are stored +1
   ...
   .IF TT_USE_OVERLAY = 1
        ...
        sta _tt_cur_ins_c0,x             ; set new instrument
        jsr forward_tt_CalcInsIndex
        lda forward_tt_InsSustainIndexes-1,y    ; -1 because instruments start at #1
        sta _tt_envelope_index_c0,x      
...
forward_tt_CalcInsIndex:
        ...
        lsr
        tay
forward_tt_Bit6Set:     ; This opcode has bit #6 set, for use with bit instruction
        rts
...
instrument:
        ...
        lda forward_tt_InsCtrlTable-1,y ; -1 because instruments start with #1
        sta AUDC0,x
        ; advance ADSR counter and compare to end of Sustain
        lda _tt_envelope_index_c0,x
        cmp forward_tt_InsReleaseIndexes-1,y    ; -1 because instruments start with #1
        bne noEndOfSustain
        ; End of sustain: Go back to start of sustain
        lda forward_tt_InsSustainIndexes-1,y    ; -1 because instruments start with #1
noEndOfSustain:
        tay
        ; Set volume from envelope
        lda forward_tt_InsFreqVolTable,y
	sta _tt_cur_vol_c0,x
